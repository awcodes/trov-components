...
<div class="relative"
    ...
    x-data="{
        open: false,
        init() {
            this.$refs.wrapper.style.width = this.$refs.wrapper.getBoundingClientRect().width + 'px';
            this.$refs.wrapper.style.height = this.$refs.wrapper.getBoundingClientRect().height + 'px';
        },
        toggle() {
    ...
    <div x-on:keydown.escape.prevent.stop="close($refs.button)"
        ...
        x-bind:class="{ 'gap-2 absolute right-0 shadow-lg': open, 'relative': !open }"
        @class([
            'inline-flex items-center justify-center font-medium tracking-tight rounded-xl border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset filament-button p-1',
